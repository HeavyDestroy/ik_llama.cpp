...
// BF16 state variant: state read/written as __nv_bfloat16, all compute in float.
// Q/K/V/G/beta remain float (intermediate compute values).
// Eliminates BF16→F32 state cast + F32→BF16 state copy-back per layer per token.
template <int HEAD_DIM, int block_size>
__global__ void delta_net_recurrent_bf16(
    const float * __restrict__ q,              // [HEAD_DIM, n_tokens, n_heads, n_seqs] F32
    const float * __restrict__ k,              // [HEAD_DIM, n_tokens, n_heads, n_seqs] F32
    const float * __restrict__ v,              // [HEAD_DIM, n_tokens, n_heads, n_seqs] F32
    const float * __restrict__ g,              // [n_tokens, 1, n_heads, n_seqs] F32
    const float * __restrict__ beta_in,        // [1, n_tokens, n_heads, n_seqs] F32
    const __nv_bfloat16 * __restrict__ state_in,  // [HEAD_DIM, HEAD_DIM*n_heads, 1, n_seqs] BF16
    __nv_bfloat16 * __restrict__ dst,             // output (F32→BF16) + new_state (BF16) concatenated
    const int64_t n_heads,
    const int64_t gqa_ratio,
    const int repeat_type,
    const int64_t n_tokens,
    const int64_t n_seqs,
    const int64_t output_offset,          // offset where state starts in output (in BF16 elements)
    size_t vnb1, size_t vnb2, size_t vnb3) {
    constexpr int warps_per_head = HEAD_DIM/WARP_SIZE;
    const int batch_idx = blockIdx.x / (warps_per_head*n_heads);
    const int sub_head_idx  = blockIdx.x % (warps_per_head*n_heads);
    ...
    const int64_t state_bf16_offset = batch_idx * state_batch_stride + state_head_offset;
    const __nv_bfloat16 * state_src = state_in + state_bf16_offset;

    // Output is BF16
    __nv_bfloat16 * out_base = dst + batch_idx * (HEAD_DIM * n_heads * n_tokens) + head_idx * HEAD_DIM;
    const int64_t out_token_stride = HEAD_DIM * n_heads;
    __nv_bfloat16 * state_dst = dst + output_offset + state_bf16_offset;
    ...
    // Load state from BF16 into float registers
    float state_local[HEAD_DIM/num_warps];
    for (int i = 0; i < HEAD_DIM/num_warps; ++i) {
        int col = num_warps*i + col_idx_0;
        state_local[i] = __bfloat162float(state_src[col*HEAD_DIM + row_out]);
    }
...
#pragma unroll
        ...
        float sv_new = beta_val * (v_ptr[t * vnb1 + row_out] - sum1 * decay);
        if (col_idx_0 == 0) {
            out_base[t * out_token_stride + row_out] = __float2bfloat16_rn(sum2 * decay + sv_new * attn_score);
        }
    ...
    __syncthreads();
    // Write final state back as BF16
    for (int i = 0; i < HEAD_DIM/num_warps; ++i) {
        int col = num_warps*i + col_idx_0;
        state_dst[col*HEAD_DIM + row_out] = __float2bfloat16_rn(state_local[i]);
    }
}

static void delta_net_bf16_cuda(
    const float * q,
    const float * k,
    const float * v,
    const float * g,
    const float * beta,
    const __nv_bfloat16 * state_in,
    __nv_bfloat16 * dst,
    ...
    if (n_tokens <= 8) {
        ...
        if (head_dim == 64) {
            delta_net_recurrent_bf16<64, threads_per_block><<<num_blocks, threads_per_block, smem_size, stream>>>(
                    q, k, v, g, beta, state_in, dst, n_heads, gqa_ratio, repeat_type, n_tokens, n_seqs, output_offset, vnb1, vnb2, vnb3);
        } else {
            delta_net_recurrent_bf16<128, threads_per_block><<<num_blocks, threads_per_block, smem_size, stream>>>(
                    q, k, v, g, beta, state_in, dst, n_heads, gqa_ratio, repeat_type, n_tokens, n_seqs, output_offset, vnb1, vnb2, vnb3);
        }
    } else {
...
void ggml_cuda_op_delta_net(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    GGML_ASSERT(head_dim <= 256);

    const int device_id = ctx.device;
    const int cc = ggml_cuda_info().devices[device_id].cc;

    if (src5->type == GGML_TYPE_BF16 && dst->type == GGML_TYPE_BF16) {
        // BF16 state I/O path: eliminates BF16↔F32 cast/cpy overhead per layer per token
        GGML_ASSERT(src5->type == GGML_TYPE_BF16);
        delta_net_bf16_cuda(
