...
extern "C" bool amx_fa_256_256(int int_type_k, int int_type_v, int nq, int nk,
...
IQK_FA_CASE(iqk_fa_256_256) {
    ...
    // Requires compiler flag: -mamd-ext-avx512
    // Currently disabled until AMX intrinsics are properly configured
    ...
    if (type_k == GGML_TYPE_BF16) {
        ...
        if (nk >= 1024 && nk%512 == 0) {
            iqk_flash_helper_T<256, 256, 512>(nq, nk, stride_q, stride_k, stride_v, stride_m, stride_qkv,
    ...
    if (nk >= 1024 && nk%512 == 0) {
        return iqk_flash_helper_T<256, 256, 512>(type_k, type_v, nq, nk, stride_q, stride_k, stride_v, stride_m, stride_qkv,
                q, ck, cv, cm, scale, softcap, qkv, sinkf, M, S);
    }
